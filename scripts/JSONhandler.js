...
class dataManager {
    createSaveFile(){//creates savefile.json if it doesn't exist.
    ...
    storage.has('savefile', (error,exists)=>{
        ...
        if(exists){
            ...
            SaveGame(savegames, false);
        }
    });
}
    SaveGame(data, recieve){ //responsibe for getting data from savefile.json and storing variables related to game progression.
        let output = {};
        storage.setDataPath(defaultsavepath);
        switch(recieve){
            case false:
